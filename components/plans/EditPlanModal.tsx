...
import { useGetPlanByIdQuery, useUpdatePlanMutation, CreatePlanData } from '@/store/api/planApi';
...
const EditPlanModal: React.FC<EditPlanModalProps> = ({
  ...
  const { data: plan, isLoading: isFetchingPlan } = useGetPlanByIdQuery(planId!, {
    skip: !planId,
  });
  ...
  // Pre-populate form with fetched plan data
  useEffect(() => {
    if (plan) {
      setFormData({
        nameEn: plan.nameEn || '',
        ...
        typePricing: plan.typePricing || {
          jewelry_shop: 300,
          jewelry_workshop: 500,
          gold_trader: 700,
          gold_distributor: 1000,
        },
        bundleDiscounts: plan.bundleDiscounts || {
          '1': 0,
          '2': 0.15,
          '3': 0.25,
          '4': 0.35,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updatePlan({ id: planId, data: formData }).unwrap();
      onSuccess();
      handleClose();
    } catch (error) {
  ...
  // Don't render modal if planId is null
  if (!planId || !isOpen) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      ...
      >
        ...
        {/* Loading state while fetching plan data */}
        {isFetchingPlan ? (
          <div className="p-6 flex items-center justify-center min-h-[200px]">
            <div className="text-center">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-amber-500 mx-auto mb-4"></div>
              <p className="text-gray-600">
                {isArabic ? 'جاري تحميل بيانات الخطة...' : 'Loading plan data...'}
              </p>
            </div>
          </div>
        ) : (
